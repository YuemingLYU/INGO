function [xmin,hist,flag]=INGOstep(N,strfitnessfct)
% INGOstep (full covaraince matrix), which is rotation-invariant
...
% test function: 'fcigar', 'ftablet', 'felli', 'felliL1', 'frastrigin10','flevy'
  ...
  while counteval < stopeval
      ...
      Zw2 = Z* bsxfun(@times,Z',Nscore);
      Zw2 = (Zw2+Zw2')/2;
      
      M = eye(N,N) + beta * Zw2;
      M = (M+ M')/2;
      
      [U,D] = eig(M);           % eigen decomposition, B==normalized eigenvectors
      D = (diag(D));
      M_invhalf  = U * diag(D.^(-1/2)) * U';
      M_invhalf = (M_invhalf+M_invhalf')/2;
      
      SIGMA_half = SIGMA_half*M_invhalf;
